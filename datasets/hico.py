...
class HICODetection(paddle.io.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries):
        ...
        self.word2vec_obj = np.load("work/hico_20160224_det/annotations/coco_clipvec.npy")
    ...
    def __getitem__(self, idx):
        ...
        if self.img_set == 'train':
            ...
            target['iscrowd'] = paddle.to_tensor([0 for _ in range(boxes.shape[0])])
            ...
            for hoi in img_anno['hoi_annotation']:
                ...
                if sub_obj_pair in sub_obj_pairs:
                    ...
                    wh_size = paddle.stack([sub_box[2] * sub_box[3], obj_box[2] * obj_box[3]])

                    obj_label = target['labels'][kept_box_indices.index(hoi['object_id'])]
                    assert 0 <= obj_label < 80
                    # print(self.word2vec_obj[obj_label])
                    # print(sub_box)
                    # print(obj_box)
                    # print(c_dis)
                    # print(wh_size.squeeze(-1))
                    gt_items_ = np.concatenate([self.word2vec_obj[obj_label],
                                                sub_box, obj_box, c_dis, wh_size.squeeze(-1)])
                    gt_items = np.concatenate([gt_items, gt_items_.reshape([1, 512 + 12])], axis=0)

            if len(sub_obj_pairs) == 0:
                target['obj_labels'] = paddle.zeros((0,), dtype=paddle.int64)
                target['verb_labels'] = paddle.zeros((0, len(self._valid_verb_ids)), dtype=paddle.float32)
                target['sub_boxes'] = paddle.zeros((0, 4), dtype=paddle.float32)
                target['obj_boxes'] = paddle.zeros((0, 4), dtype=paddle.float32)
                target['gt_items'] = paddle.zeros((0, 512+12), dtype=paddle.float32)
            else:
                target['obj_labels'] = paddle.stack(obj_labels)
                target['verb_labels'] = paddle.to_tensor(verb_labels, dtype=paddle.float32)
                target['sub_boxes'] = paddle.stack(sub_boxes)
                target['obj_boxes'] = paddle.stack(obj_boxes)
                target['gt_items'] = paddle.to_tensor(gt_items, dtype=paddle.float32)
        else:
            target['boxes'] = boxes
            target['labels'] = classes
            target['id'] = idx
            ...
            target['hois'] = paddle.to_tensor(hois, dtype=paddle.int64)
    ...
    def get_sim_index(self):
        self.sim_index = pickle.load(open('work/hico_20160224_det/'
