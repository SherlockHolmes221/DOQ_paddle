...
def crop(image, target, region):
    ...
    if "boxes" in target:
        ...
        cropped_boxes = paddle.fmin(cropped_boxes.reshape([-1, 2, 2]), max_size)
...
def hflip(image, target):
    ...
    if "boxes" in target:
        boxes = target["boxes"]
        boxes = paddle.concat([boxes[:,2:3],boxes[:,1:2],boxes[:,0:1],boxes[:,3:4]], axis=-1)
        boxes = boxes * paddle.to_tensor([-1, 1, -1, 1]) + paddle.to_tensor([w, 0, w, 0])
        target["boxes"] = boxes
...
class RandomCrop(object):
    ...
    def __call__(self, img, target):
        region = F.RandomCrop(size=self.size)._get_params(img, self.size)
...
class RandomSizeCrop(object):
    ...
    def __call__(self, img: PIL.Image.Image, target: dict):
        ...
        region = F.RandomCrop(size=[h, w])._get_param(img, [h, w])
        return crop(img, target, region)
...
class RandomErasing(object):
    ...
    def __init__(self, *args, **kwargs):
        self.eraser = F.RandomErasing(*args, **kwargs)
...
class ColorJitter(object):
    def __init__(self, brightness=0, contrast=0, saturatio=0, hue=0):
        self.color_jitter = F.ColorJitter(brightness, contrast, saturatio, hue)
