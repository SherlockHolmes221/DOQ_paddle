...
class HungarianMatcherHOI(paddle.nn.Layer):
    ...
    def forward(self, outputs, targets):
        ...
        tgt_obj_labels = paddle.concat([v['obj_labels'] for v in targets])
        tgt_verb_labels = paddle.concat([v['verb_labels'] for v in targets])
        tgt_verb_labels_permute = tgt_verb_labels.permute(1, 0)
        tgt_sub_boxes = paddle.concat([v['sub_boxes'] for v in targets])
        tgt_obj_boxes = paddle.concat([v['obj_boxes'] for v in targets])
        ...
        cost_sub_bbox = paddle.dist(out_sub_bbox, tgt_sub_boxes, p=1)
        cost_obj_bbox = paddle.dist(out_obj_bbox, tgt_obj_boxes, p=1) * (tgt_obj_boxes != 0).any(dim=1).unsqueeze(0)
        if cost_sub_bbox.shape[1] == 0:
            cost_bbox = cost_sub_bbox
        else:
            cost_bbox = paddle.stack((cost_sub_bbox, cost_obj_bbox)).max(dim=0)[0]
        ...
        if cost_sub_giou.shape[1] == 0:
            ...
            cost_giou = paddle.stack((cost_sub_giou, cost_obj_giou)).max(dim=0)[0]
